...
Microsoft::WRL::ComPtr<ID3D12Resource> YTEngine::CreateTextureResource(ID3D12Device* device, const DirectX::TexMetadata& metadata) {
...
Microsoft::WRL::ComPtr<ID3D12Resource> YTEngine::UploadTextureData(ID3D12Resource* texture, const DirectX::ScratchImage& mipImages, uint32_t index) {
...
DirectX::ScratchImage YTEngine::LoadTexture(const std::string& filePath) {
...
void YTEngine::LoadTexture(const std::string& filePath, uint32_t index) {
...
D3D12_CPU_DESCRIPTOR_HANDLE YTEngine::GetTextureSrvHandleCPU(ID3D12DescriptorHeap* descriptorHeap, uint32_t descriptorSize, uint32_t index) {
	D3D12_CPU_DESCRIPTOR_HANDLE handleCPU = descriptorHeap->GetCPUDescriptorHandleForHeapStart();
	handleCPU.ptr += (descriptorSize * index);
	return handleCPU;
}

D3D12_GPU_DESCRIPTOR_HANDLE YTEngine::GetTextureSrvHandleGPU(ID3D12DescriptorHeap* descriptorHeap, uint32_t descriptorSize, uint32_t index) {
